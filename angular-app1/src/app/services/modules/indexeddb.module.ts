// https://github.com/ngrx/db/blob/master/src/database.ts
// RxJs 6 version : https://github.com/brandonroberts/ngrx-db-builds/blob/f82cf46e699ba9ac46cec2f586caa392a814814f/src/database.js
...
export class Database {
    ...
    insert(
        ...
        return write$.pipe(
            tap(payload => {
                return notify
                    ? this.changes.next({ type: DB_INSERT, payload: payload })
                    : {};
            })
        );
    }
...
export class DBModule {
    static provideDB(schema: DBSchema): ModuleWithProviders<DBModule> {
        return {
            ngModule: DBModule,
...
// 2. import export it
export { Database, DBSchema, DBStore, DBUpgradeHandler } from './indexeddb-database';
 
 
// 3. Then in your component ngOnInit method use below code
this.database.open('MyDB').subscribe(data => {
    // Add new record
    this.database.insert('footer', [{ id: "01", name: "prasad", age: 24, email: "prasad@tutorialspoint.com" }], true).subscribe(data1 => {
        console.log('IDB insert: ', data1);
    });
